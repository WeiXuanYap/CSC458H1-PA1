...
void handle_arpreq(struct sr_instance *sr, struct sr_arpreq *req) {
  time_t curr_t;
  time(&curr_t);
  if (difftime(curr_t, req->sent) >= 1.0) {
    if (req->times_sent >= 5) {
      ...
      arp_frame_ahdr->ar_hrd = (unsigned short)htons(arp_hrd_ethernet);
      arp_frame_ahdr->ar_pro = (unsigned short)htons(ethertype_ip);
      arp_frame_ahdr->ar_hln = (unsigned char)ETHER_ADDR_LEN;
...
void sr_arpcache_sweepreqs(struct sr_instance *sr) {
  /* iterate through list of arp reqs and decide what to do with them */
